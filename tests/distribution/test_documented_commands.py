...
LINUX_INSTALL_COMMAND = "./install.sh --version 1.0.10 --yes"
MACOS_INSTALL_COMMAND = "./install.sh --version 1.0.10 --yes"
...
def test_documented_lifecycle_commands_match_platform_grammar() -> None:
    ...
    for commands in (readme, installation):
        ...
        assert r".\install.ps1 -Version 1.0.10" not in commands
        assert "./install.sh --version 1.0.10" not in commands
    assert {"goalrouter update", "goalrouter update 1.0.10"} <= upgrading
    assert {
