...
def _assert_publish_policy(workflow: Mapping[str, Any]) -> None:
    ...
    assert version_run == "\n".join(
        [
            ...
            "  --version 1.0.4 \\",
            '  --tag "$GITHUB_REF_NAME" \\',
            "  --image ghcr.io/vparla/goalrouter:1.0.4 \\",
    ...
    assert 'test "$GITHUB_REF_NAME" = v1.0.4' in guard_run
    assert "for tag in v1.0.4 1.0.4; do" in guard_run
    assert "for tag in v1.0.4 1.0.4 1.0" not in guard_run
    ...
    assert "prior_digest=$(resolve_existing_digest 1.0.3 prior-immutable)" in guard_run
    ...
    assert "moving GHCR alias diverged from 1.0.3" in guard_run
    ...
    assert "for tag in v1.0.4 1.0.4 1.0 1 latest; do" in stable_aliases_run
    ...
    expected_assets = [
        "SHA256SUMS",
        "goalrouter-1.0.4-unix.tar.gz",
        "goalrouter-1.0.4-windows.zip",
        "install.ps1",
        "install.sh",
...
def test_publish_policy_rejects_tag_attestation_release_and_action_mutations() -> None:
    ...
    guard["run"] = guard["run"].replace(
        "for tag in v1.0.4 1.0.4; do", "for tag in v1.0.4; do"
    )
    with pytest.raises(AssertionError):
        _assert_publish_policy(workflow)
