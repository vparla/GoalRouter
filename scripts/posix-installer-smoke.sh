...
docker container create \
    ...
    goalrouter-installer-smoke:local -eu -c '
        ...
        tar -czf /release/goalrouter-1.0.6-unix.tar.gz -C /tmp/release-asset goalrouter install.sh uninstall.sh
        cd /release
        sha256sum goalrouter-1.0.6-unix.tar.gz >SHA256SUMS
...
registry_reference=127.0.0.1:$registry_port/goalrouter:1.0.6
...
docker run --rm \
    ...
    --entrypoint /bin/sh \
    goalrouter-installer-smoke:local -eu -c '
        printf '\''{"version":"1.0.6","protocol_version":1,"image":"%s","image_digest":"%s","architectures":["linux/amd64","linux/arm64"],"source_revision":"posix-installer-smoke","minimum_hosts":{"windows":"10.0.19045","powershell":"5.1","wsl":"2.2.3","docker":"20.10"}}\n'\'' "$1" "$2" >/release/release-manifest.json
        cd /release
        sha256sum release-manifest.json goalrouter-1.0.6-unix.tar.gz >SHA256SUMS
...
set -eu
        ...
        grep -Fxq launcher_version=1.0.6 /tmp/installed-version
        ...
        grep -Fxq launcher_version=1.0.6 /tmp/updated-version
        "$HOME/.local/bin/goalrouter" doctor --skip-account
        "$HOME/.local/bin/goalrouter" uninstall --yes
        test ! -e "$HOME/.local/bin/goalrouter"
